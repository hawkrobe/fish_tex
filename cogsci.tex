...
We recruited 781 unique participants from Amazon Mechanical Turk to
participate in this experiment.  All participants were from the United
States.  After excluding 52 participants due to inactivity or latency,
and 9 others for disconnecting in the first half of the game, we were
left with usable data from 720 participants in 312 groups.  These
groups ranged in size from one to six individuals. 
...
\includegraphics[width=0.3\textwidth]{./figures/experiment-3-wall}
  ...
  \hspace{0.1cm}
  \todo[inline]{TODO: Create a readable schematic with a blown-up screenshot in supplemental.}
...
game.  We created these score fields using a simple process.  We first
generated a circular scoring region of 50 pixels at a random location on the playing area. This circle then traversed in straight lines to randomly chosen
next locations within the playing area at a speed of 136 pixels per second.
...
We designed the participant interface to be a kind of multiagent tracking environment. The underlying score field was kept hidden from
participants, and instead the participants could only detect when
they were within the circular scoring region. When within the moving circular region, the participant avatar was surrounded on their screen by a sparkling star and the border of the playing area turned yellow. The sparkling star was only visible to the participant controlling that avator. Participants \emph{not} directly observe whether other players were in the scoring region.

Players controlled their avatars by clicking and using two keyboard keys. Clicking within the playing area instantly oriented the direction
of the avatar to be facing the location clicked. Participants could hold the
"a" key to accelerate or hold down the "s" key to stop.  The avatars automatically moved forward at a
constant velocity of 136 pixels per second whenever neither the "a" key nor the "s" key was
...
"a" key was held down, and instantaneously decreased to no velocity or 0 pixels per second for the duration of time the "s" key was held down. Each participant played in a single
continuous game lasting for 5 minutes. Participants 1/400 point for each second active in the game (not against a wall) and an additional 1/600 point per second whenever in the scoring region.
...
described the mechanics of the game, framing the game as searching for the "magical bonus region".  The participants were
informed about the dynamics of the underlying score fields.  The  were also informed "There is no competition between players; the magical region is not consumed by players. It simply changes location over time." Participants were not explicitly instructed or suggested to cooperate or coordinate with each other.
...
\$1.25 during the five minutes of gameplay. Each point in the game corresponds to \$0.01 of bonus. Each participant was also paid 15 cents
...
\cite{hawkins_conducting_2014}.  The MWERT framework uses a set of
recent web technologies capable of handling the challenges of
real-time, multi-player web experiments, including Node.js, the
Socket.io module, and HTML5 canvases.  Since MWERT was originally used
