...
\todo[inline]{rxdh: do we also want to report the significant interaction with the non-social condition here (i.e. that this selectivity was not observed with the `invisible' bots, making it unlikely to observe such a pattern by chance). Otherwise, we're not actually using the non-social condition anywhere in the results.}
...
One particular score field in the high condition displayed a significant effect of group size, while none of the others do. \todo[inline]{Check this.}
...
\todo[inline]{rxdh: first, let's report a regression here on state proportions? And show a plot where the low \& high noise, and all groups sizes are collapsed together?}
...
\todo[inline]{rxdh: second, let's break it out by noise and group size, and analyze how these may affect strategies, suggest this may help explain gaps in scores?}
Second paragraph with results here.

\todo[inline]{rxdh: third, we can zoom into properties of copying behavior more specifically, and revise this paragraph with the updated results (e.g. we took out the correlations of higher performance with better copying; do we want to replace it with something?)}
...
\todo[inline]{should we make sure to discuss this above? we could just add it as an earlier general discussion paragraph... or combine this section with non-human mechanisms?}
